...
{
	$error = 0;
	$select = 0; //savoir si une piece a été selectionnée
	...
	if (! $_POST["cancel"]){
		...
		//aucune piece n'a ete selectionnée => erreur
		if($select== 0){
			$msg = '<div class="error">Aucune piece n\'a ete selectionnée</div>';
			$error++;
		}
	}
}
...
llxHeader('','Ajout Piece à Ballon','');
if($msg){
	print $msg;
}
$form=new Form($db);
